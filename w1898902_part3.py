...
list_1 = []                                 # create the list as a tempory list
while True:
    while True:
            try:
    ...
    while True:
              ...
              if total > 120 or total< 120:                                                                                                      # check total calculation for credits 
                  ...
                  retriever = f"{no_of_credits_at_pass},{no_of_credits_at_defer},{no_of_credits_at_fail}"              # assign credits to variable retriever
    ...
    while True:
            ...
            if option == "y":                                        # enter y if want to continue
                break
                    ...
                    credit_name = ['progress','trailer','retriever','exclude']                                 # created a list including progression to print for vertical histogram
                    print(credit_name[0],credit_name[1],credit_name[2],credit_name[3])                      #  print outcomes using indexing
                    for x in range(max(progress_count,trailer_count,retriever_count,exclude_count)):              # got each range of progression for using print stars
            ...
            for i in range (0,len(list_1),2) :                            # got range using the length of tempory list
                credit = list_1[i] + list_1[i+1]                          #  After I broke from large temp list to small lists for print 
